...
while read evtfile; do # loop over all virtual evts
  ...
  if $is_ls;then
    cat ../src_rec/STATIONS_$eid |awk '{printf"../%s/%s.%s.HXZ.sac\n",a,$2,$1}' a=$datadir >dat.lst
    for datf in `cat dat.lst`;do
      prefix=`echo $datf |awk -F../${datadir}/ '{print $2}' |awk -F.sac '{print $1}'`
      stnm=`echo $prefix |awk -F. '{printf"%s.%s\n",$1,$2}'`
      if [ ! -f $eid/DATA_NORM/${prefix}.sac ];then # if COR_STA1_STA2 not found, copy from COR_STA2_STA1 
         cp ../data/$stnm/$eid.HXZ.sac $eid/DATA_NORM/${prefix}.sac
      fi
    done    
  else  #### only do measurement on CN2 
    ls ../${datadir}/*.sac > dat.lst
  fi
  for datf in `cat dat.lst`;do
    prefix=`echo $datf |awk -F../${datadir}/ '{print $2}' |awk -F.sac '{print $1}'`
...
###---end of prepare_measure_adj.pl---###
   if $is_bd;then
    ...
    if $i_plot;then
      ...
      if $is_ls;then  ### for line search, do not calculate adjoint sources
...
if $is_ls;then
   echo "line search without adjoint sources"
else
#------- write SEM
echo "cd ../../" >>../job$i.sh
#-------
