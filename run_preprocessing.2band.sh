...
# for two period bands. The output two directories are:
# seis_process_M??.set?
# measure_adj_M00.set?
...
for T in T005_T010 T010_T020 T020_T050;do
...
###---end of prepare_measure_adj.pl---###
   if $is_bd;then
    ...
    if $i_plot;then
      ...
      elif [ $T == "T010_T020" ];then 
         echo "./run_measure_adj.pl $mod -20/235 0 0 0 -2.0/0.01/24000 7 HX $hp/$lp 0/0/0/1 -3.5/3.5/-1.0/1.0/0.75 1/1.0/0.5 1/0.02/2.5/2.0/2.5/3.5/1.5 >>run.log" >>../job$i.sh
      elif [ $T == "T020_T050" ];then
         echo "./run_measure_adj.pl $mod -20/235 0 0 0 -2.0/0.01/24000 7 HX $hp/$lp 0/0/0/1 -4.5/4.5/-1.0/1.0/0.75 1/1.0/0.5 1/0.02/2.5/2.0/2.5/3.5/1.5 >>run.log" >>../job$i.sh
      fi 
    fi
   fi
  # 
...
echo "./combine_2_adj_src.pl HX ADJOINT_SOURCES_T010_T020 ADJOINT_SOURCES_T020_T050 ADJOINT_SOURCES iker07 iker07 >>run.log " >>../job$i.sh
