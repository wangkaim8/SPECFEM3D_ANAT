...
while read evtfile; do # loop over all virtual evts
  ...
  for datf in `ls ../${datadir}/*.sac`;do
    ...
    for band in T005_T050 T010_T050 T020_T050;do
...
cp ../../ADJOINT_TOMOGRAPHY_TOOLS/measure_adj/scripts_tomo/combine_2_adj_src.pl .
...
#---write MEASURE.WINDOWS for the next step
   ...
   for rawsac in `ls DATA/*sac`;do
      ...
      echo $datsac >>MEASUREMENT.WINDOWS.$T
      echo $synsac >>MEASUREMENT.WINDOWS.$T
      echo 1 >>MEASUREMENT.WINDOWS.$T
      echo $R1_time $R0_time >>MEASUREMENT.WINDOWS.$T
   done
   echo "cp MEASUREMENT.WINDOWS.$T MEASUREMENT.WINDOWS" >>../job$i.sh
#--- for plot
#  if $i_plot;then
#  rm -rf PLOTS/SYN
#  mkdir PLOTS/SYN
...
###---end of prepare_measure_adj.pl---###
   if $is_bd;then
    ...
    if $i_plot;then
      ...
      if [ $T == "T005_T050" ];then 
         echo "./run_measure_adj.pl $mod -20/235 0 0 0 -2.0/0.01/24000 7 HX $hp/$lp 0/0/0/1 -2.5/2.5/-1.0/1.0/0.80 1/1.0/0.5 1/0.02/2.5/2.0/2.5/3.5/1.5 >>run.log" >>../job$i.sh
      elif [ $T == "T010_T050" ];then 
         echo "./run_measure_adj.pl $mod -20/235 0 0 0 -2.0/0.01/24000 7 HX $hp/$lp 0/0/0/1 -3.5/3.5/-1.0/1.0/0.80 1/1.0/0.5 1/0.02/2.5/2.0/2.5/3.5/1.5 >>run.log" >>../job$i.sh
      elif [ $T == "T020_T050" ];then
         echo "./run_measure_adj.pl $mod -20/235 0 0 0 -2.0/0.01/24000 7 HX $hp/$lp 0/0/0/1 -4.5/4.5/-1.0/1.0/0.80 1/1.0/0.5 1/0.02/2.5/2.0/2.5/3.5/1.5 >>run.log" >>../job$i.sh
      fi 
    fi
   fi
  # 
   echo "mv ADJOINT_SOURCES ADJOINT_SOURCES_$T" >>../job$i.sh
   if $i_plot;then
...
echo "./combine_3_adj_src.pl HX ADJOINT_SOURCES_T005_T050 ADJOINT_SOURCES_T010_T050 ADJOINT_SOURCES_T020_T050 ADJOINT_SOURCES iker07.density iker07.density iker07.density >>run.log " >>../job$i.sh
